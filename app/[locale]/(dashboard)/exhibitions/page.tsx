...
export default function ExhibitionsPage() {
  ...
  const displayLocation = (e: ExhibitionRow) => ((locale === 'en' && e.location_en ? e.location_en : e.location_ar) || '')
  ...
  const openEditModal = (ex: ExhibitionRow) => {
    setEditingExhibition(ex)
    const isFixedCity = (CITY_VALUES as readonly string[]).includes(ex.city || '')
    setForm({
      title_ar: ex.title_ar || '',
      title_en: ex.title_en || '',
      description_ar: ex.description_ar || '',
      description_en: ex.description_en || '',
      city: ex.city ? (isFixedCity ? ex.city : 'custom') : 'الرياض',
      customCity: ex.city && !isFixedCity ? ex.city : '',
      location_ar: ex.location_ar || '',
      location_en: ex.location_en || '',
  ...
  const handleSave = async () => {
    ...
    const { error } = editingExhibition
      ? await supabase.from('exhibitions').update(payload).eq('id', editingExhibition.id)
    ...
    flashSuccess(editingExhibition ? t('saveSuccess') : t('addSuccess'))
  }

  const handleDelete = async (ex: ExhibitionRow) => {
    if (!window.confirm(t('deleteConfirm', { title: displayTitle(ex) }))) return
  ...
  return (
    <div>
      ...
      <div style={{ padding: isMobile ? '16px' : '28px 32px', direction: isRtl ? 'rtl' : 'ltr' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '64px 24px', color: 'var(--text-muted)' }}>
            ...
            <p style={{ fontSize: '14px' }}>{exhibitions.length === 0 ? t('emptyState') : t('noResultsMessage')}</p>
          </div>
        ) : (
                ...
                <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {ex.city && <Badge text={ex.city} variant={cityVariant(ex.city)} />}
                    </div>
                    <PersonCardMenu
                      isRtl={isRtl}
                  ...
                  {displayLocation(ex) && (
                    <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{displayLocation(ex)}</div>
                  )}
                  ...
                  {(ex.contact_email || ex.contact_phone) && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 4 }}>
                      {ex.contact_name && (
                        <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{ex.contact_name}</span>
                      )}
                      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                        {ex.contact_email && (
                          <a href={`mailto:${ex.contact_email}`} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '12px', color: 'var(--gold-dark)', textDecoration: 'none' }}>
                            <Mail size={12} /> {ex.contact_email}
                          </a>
                        )}
                        {ex.contact_phone && (
                          <a href={`tel:${ex.contact_phone}`} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '12px', color: 'var(--gold-dark)', textDecoration: 'none' }}>
                            <Phone size={12} /> {ex.contact_phone}
                          </a>
                        )}
                      </div>
                    </div>
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={editingExhibition ? t('editModalTitle') : t('addModalTitle')}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
            ...
            >
              {saving ? t('saving') : editingExhibition ? t('saveButtonEdit') : t('saveButton')}
            </button>
            <button
              onClick={closeModal}
              style={{
